...
def analyse_track(file, output):
	...
	with open(outputfile) as datafile:
		...
		silence20 = data["lowlevel"]["silence_rate_20dB"]["mean"]
		silence30 = data["lowlevel"]["silence_rate_30dB"]["mean"]
		silence60 = data["lowlevel"]["silence_rate_60dB"]["mean"]
		dynamic_complexity = data["lowlevel"]["dynamic_complexity"]
		bmp = data["rhythm"]["bpm"]
		chords_change = data["tonal"]["chords_changes_rate"]
		chords_key = chords_lookup[data["tonal"]["chords_key"]]
		chords_scale = data["tonal"]["chords_scale"] == "major"
		key_key = chords_lookup[data["tonal"]["key_key"]]
		key_scale = data["tonal"]["key_scale"] == "major"

		output.write(file_name + '\t' + str(loudness) + '\t' + str(silence20) + '\t' + str(silence30) + '\t' \
						+ str(silence60) + '\t' + str(dynamic_complexity) + '\t' + str(bmp) + '\t' + str(chords_change) + '\t' \
